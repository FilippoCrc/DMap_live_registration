...
void DMapLocalizer::setMap(const GridMapping& mapping,const Grid_<float>& distances_) {
...
void DMapLocalizer::setMap(const std::vector<Eigen::Vector2f> obstacles,float resolution,float influence_range) {
...
// initialization function, from map   VOGLIO USARE QUESTA
...
bool DMapLocalizer::localize(const std::vector<Vector2f>& measurements, int iterations) {
  int inliers=0;
  float chi2=0;
